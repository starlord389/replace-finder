...
export default function AdminRepresentations() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h1 className="text-2xl font-bold">Representation Operations</h1><p className="mt-1 text-sm text-muted-foreground">Assign referrals, monitor invitations, and resolve investor-agent access issues.</p></div><Button variant="outline" onClick={runE2eDiagnostic} disabled={busy === "representation-e2e"}>{busy === "representation-e2e" ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <TestTube2 className="mr-2 h-4 w-4" />}{busy === "representation-e2e" ? "Running isolated test…" : "Run multi-account test"}</Button></div>
