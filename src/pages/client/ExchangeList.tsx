    ...
    <div className="max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Manage your 1031 exchange requests.</p>
        </div>
      ...
      {requests.length === 0 ? (
        <div className="mt-8 rounded-xl border bg-card p-12 text-center">
          <p className="text-muted-foreground">You haven't submitted an exchange request yet.</p>
          <Link to="/dashboard/exchanges/new" className="mt-4 inline-block">
        ...
        <div className="mt-6 space-y-3">
          {requests.map((req) => (
            <Link
              key={req.id}
              to={`/dashboard/exchanges/${req.id}`}
              className="group flex items-center justify-between rounded-xl border bg-card p-5 transition-all hover:border-primary/30 hover:shadow-md"
            >
