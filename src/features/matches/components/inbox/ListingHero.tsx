...
export function ListingHero({ rel }: Props) {
  ...
  const heroSrc = photos.length ? photos[Math.min(active, photos.length - 1)] : null;
  const location = [rel.propertyCity, rel.propertyState].filter(Boolean).join(", ") || "Location available on request";
