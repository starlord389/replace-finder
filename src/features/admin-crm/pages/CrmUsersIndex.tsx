...
import { getAdminOnboardingStage, type AdminOnboardingStage } from "@/features/admin/hooks/useAdminUserDirectory";
...
const PAGE_SIZE = 25;
const USER_DIRECTORY_GRID = "grid-cols-[minmax(220px,1.15fr)_minmax(145px,.65fr)_minmax(175px,.85fr)_minmax(165px,.75fr)_minmax(175px,.8fr)_minmax(105px,.5fr)_minmax(130px,.6fr)_40px]";
...
export default function CrmUsersIndex() {
  ...
  return (
    <div className="space-y-6">
      <CrmPageHeader eyebrow="CRM directory" title="People" description="Every real signup appears here, including people who never confirmed their email or created any workspace data. Open an account to see its complete hierarchy and onboarding history." actions={<Button variant="outline" size="sm" onClick={() => directory.refetch()} disabled={directory.isFetching}><RefreshCw className={`mr-2 h-4 w-4 ${directory.isFetching ? "animate-spin" : ""}`} />Refresh</Button>} />
      <div className="grid gap-3 sm:grid-cols-3">
        <MetricTile label={isDemo ? "Demo users" : "Real signups"} value={summary?.totalAccounts ?? "—"} icon={Users} detail={isDemo ? "Accounts connected to sample data" : "Includes zero-activity accounts"} tone="blue" />
        ...
        {directory.isError ? <div className="p-5"><CrmError title="Users could not be loaded" message={directory.error instanceof Error ? directory.error.message : "Try again."} onRetry={() => { void directory.refetch(); }} /></div>
          : directory.isLoading ? <div className="p-5"><CrmLoading rows={8} /></div>
            : users.length === 0 ? <div className="p-14 text-center"><Search className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 font-medium text-slate-800">No users match these filters</p><p className="mt-1 text-sm text-slate-500">Try a broader search.</p></div>
              : <div className="overflow-x-auto"><table className="w-full min-w-[1180px] text-left"><thead className="bg-slate-50 text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-500"><tr className={`grid ${USER_DIRECTORY_GRID}`}><th className="px-5 py-3">User</th><th className="px-4 py-3">Account type</th><th className="px-4 py-3">Organization & market</th><th className="px-4 py-3">Workspace</th><th className="px-4 py-3">Journey stage</th><th className="px-4 py-3">Access</th><th className="px-4 py-3">Last activity</th><th aria-label="Open user" /></tr></thead><tbody className="divide-y divide-slate-100">{users.map((user) => { const journey = getAdminOnboardingStage(user); return (
                <tr key={user.id} className="group transition hover:bg-slate-50/80"><td className="p-0" colSpan={8}><Link to={`/admin/users/${user.id}`} state={{ adminReturnTo: `${location.pathname}${location.search}` }} className={`grid ${USER_DIRECTORY_GRID} items-center`}>
                  ...
                  <div className="px-4 py-4"><JourneyStage stage={journey} /></div>
...
function JourneyStage({ stage }: { stage: AdminOnboardingStage }) {
  const tone = {
  ...
  return <div><span className={`inline-flex rounded-full border px-2 py-1 text-[10px] font-semibold ${tone}`}>{stage.label}</span><p className="mt-1.5 line-clamp-2 text-[10px] leading-4 text-slate-500">{stage.detail}</p></div>;
}
