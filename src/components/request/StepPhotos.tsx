...
export default function StepPhotos({ requestId, images, setImages }: Props) {
  const { user } = useAuth();
  ...
  const uploadFiles = useCallback(async (files: FileList | File[]) => {
    ...
    setUploading(false);
  }, [user, images, setImages, toast]);

  const removeImage = async (idx: number) => {
    const img = images[idx];
    await supabase.storage.from("request-images").remove([img.storage_path]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-dashed border-primary/30 bg-primary/5 p-4">
        <p className="text-sm text-primary">📷 Photos help our team better understand your property and find the right match.</p>
      </div>
