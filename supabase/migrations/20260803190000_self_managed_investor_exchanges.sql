-- Self-managed investor/property-owner exchanges.
--
-- The existing exchange engine stores the authenticated participant in
-- exchanges.agent_id / pledged_properties.agent_id. Those ids are also used by
-- match visibility, conversations, and notifications, so they remain the
-- participant/contact ids for backwards compatibility. owner_type distinguishes
-- an agent-managed exchange from an investor managing their own property.
...
-- Make the ownership intent explicit in RLS. Existing rows are agent-owned.
...
-- Investors (and agents) may see their own listings plus only the active
-- properties that the matching engine has actually matched to one of their
-- exchanges. This removes the global authenticated-user marketplace.
...
-- Close the same global-listing gap on the supporting property tables. A user
-- can read a candidate's details only after a match exists (or while connected).
