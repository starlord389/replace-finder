...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Load the activated exchange + criteria + property + financials
    const [exchangeRes, propertyRes] = await Promise.all([
      db.from("exchanges").select("*").eq("id", exchange_id).single(),
      db.from("pledged_properties").select("*").eq("id", property_id).single(),
    ]);
    if (exchangeRes.error || !exchangeRes.data) return jsonResponse({ error: "Exchange not found" }, 404);
    if (propertyRes.error || !propertyRes.data) return jsonResponse({ error: "Property not found" }, 404);

    const exchange = exchangeRes.data;
    const property = propertyRes.data;

    const [criteriaRes, propFinRes] = await Promise.all([
      exchange.criteria_id
        ? db.from("replacement_criteria").select("*").eq("id", exchange.criteria_id).single()
        : Promise.resolve({ data: null, error: null }),
      db.from("property_financials").select("*").eq("property_id", property_id).maybeSingle(),
    ]);

    const criteria = criteriaRes.data;
    const propertyFin = propFinRes.data;

    const allMatches: any[] = [];

    // === BUYER-SIDE: find properties FOR this exchange ===
    if (criteria) {
      const { data: activeProps } = await db
      ...
      if (activeProps && activeProps.length > 0) {
        ...
        const { data: allFins } = await db
          .from("property_financials")
          .select("*")
          .in("property_id", propIds);

        const finsMap = new Map((allFins || []).map((f: any) => [f.property_id, f]));

        for (const candidateProp of activeProps) {
          const candidateFin = finsMap.get(candidateProp.id);
          const scores = scoreProperty(candidateProp, candidateFin, criteria, exchange);
          if (scores.total >= MATCH_THRESHOLD) {
            const boot = calculateBoot(exchange, propertyFin, candidateProp, candidateFin);
            allMatches.push({
              buyer_exchange_id: exchange_id,
              seller_property_id: candidateProp.id,
              ...scores,
              ...boot,
              status: "active",
              _direction: "buyer",
              _other_agent_id: candidateProp.agent_id,
            });
          }
        }
      }
    ...
    // === SELLER-SIDE: find exchanges that want THIS property ===
    ...
    if (otherExchanges && otherExchanges.length > 0) {
      // Load financials for the other exchanges' relinquished properties
      const relinquishedIds = otherExchanges
        .map((e: any) => e.relinquished_property_id)
        .filter(Boolean);
      const { data: otherFins } = relinquishedIds.length > 0
        ? await db.from("property_financials").select("*").in("property_id", relinquishedIds)
        : { data: [] };
      const otherFinsMap = new Map((otherFins || []).map((f: any) => [f.property_id, f]));
      ...
      for (const otherExchange of otherExchanges) {
        ...
        if (scores.total >= MATCH_THRESHOLD) {
          const otherRelinquishedFin = otherExchange.relinquished_property_id
            ? otherFinsMap.get(otherExchange.relinquished_property_id)
            : null;
          const boot = calculateBoot(otherExchange, otherRelinquishedFin, property, propertyFin);
          allMatches.push({
            ...
            status: "active",
            _direction: "seller",
            _other_agent_id: otherExchange.agent_id,
          });
        }
      }
    }
    ...
    if (allMatches.length > 0) {
      ...
      const { data: upserted, error: upsertErr } = await db
        .from("matches")
        .upsert(matchRows, { onConflict: "buyer_exchange_id,seller_property_id" })
        .select("id");

      if (upsertErr) {
        console.error("Upsert error:", upsertErr);
      } else {
        newMatchCount = upserted?.length || 0;
      }
    }
    ...
    for (const m of allMatches) {
      if (m._direction === "buyer") {
        // Notify current user (buyer agent) about found property
        notifications.push({
          user_id: userId,
          type: "new_match",
          title: "New Match Found",
          message: "A property matching your client's criteria has been found in the network.",
          link_to: "/agent/matches",
        });
      } else {
        // Notify the other agent (buyer) that a new property matches their criteria
        notifications.push({
          user_id: m._other_agent_id,
          type: "new_match",
          title: "New Match Found",
          message: "A new property has entered the network that matches your client's criteria.",
          link_to: "/agent/matches",
        });
      }
    }
    ...
    return jsonResponse({
      matches_for_exchange: allMatches.filter((m) => m._direction === "buyer").length,
      matches_from_property: allMatches.filter((m) => m._direction === "seller").length,
...
function scoreProperty(
  ...
  const total =
    ...
    debtFit * WEIGHTS.debtFit +
    scaleFit * WEIGHTS.scaleFit;
...
function scoreGeo(prop: any, criteria: any): number {
  ...
  let score = 0;
  if (hasStates && prop.state) {
    if (criteria.target_states.includes(prop.state)) score += 70;
  }
  if (hasMetros && prop.city) {
    const cityLower = prop.city.toLowerCase();
    if (criteria.target_metros.some((m: string) =>
      cityLower.includes(m.toLowerCase()) || m.toLowerCase().includes(cityLower)
    )) {
      score += 30;
    }
  }
...
function scoreFinancial(prop: any, fin: any, criteria: any): number {
  ...
  let bonus = 0;
  if (criteria.target_occupancy_min && fin?.occupancy_rate) {
    if (Number(fin.occupancy_rate) >= Number(criteria.target_occupancy_min)) bonus += 20;
  }
  if (criteria.target_year_built_min && prop.year_built) {
    if (Number(prop.year_built) >= Number(criteria.target_year_built_min)) bonus += 10;
  }
...
function scoreScaleFit(prop: any, criteria: any): number {
  ...
  let bonus = 0;
  if (criteria.target_property_classes?.length && prop.property_class) {
    if (criteria.target_property_classes.includes(prop.property_class)) bonus += 20;
  }
...
function calculateBoot(
  ...
  const bootTax = totalBoot * 0.30;

  let bootStatus: string;
  if (totalBoot === 0) {
    bootStatus = "no_boot";
  } else if (proceeds > 0 && totalBoot < proceeds * 0.05) {
    bootStatus = "minor_boot";
  } else {
    bootStatus = "significant_boot";
  }
